...
const NewBadge = styled.div`
  ...
  font-style: italic;
`;
const SliderControlsOverlay = styled.div`
  position: absolute;
  bottom: 30px;
  left: 50%;
...
const ColorThumbnail = styled.img`
  ...
  height: 45px;
  object-fit: cover;
  border: 1px solid ${(props) => (props.$active ? "#212121" : "#eee")};
  padding: 2px;
  cursor: pointer;
  background: #f5f5f5;
...
const ProductDetailPage = () => {
  ...
  const handleNext = () =>
    setActiveIndex((prev) => (prev + 1) % LOCAL_COLORS.length);
  const handlePrev = () =>
    setActiveIndex(
      (prev) => (prev - 1 + LOCAL_COLORS.length) % LOCAL_COLORS.length
    );
  ...
  const accordionData = [
    ...
    },
    {
      ...
      content: (
        <p>
          지속 가능성은 무슨 환경보다 본인의 학접부터 신경쓰는게 좋지 않을까요?
        </p>
      ),
    },
    {
      id: "care",
      title: "케어 방법",
      content: <p>신발을 닦을 시간이 있다면 나가서 돈이나 버는게.....</p>,
    },
    {
      id: "shipping",
      title: "배송 & 반품",
      content: <p>그 여자친구는 반송안됩니다.</p>,
    },
  ];
        ...
        <LeftSection>
          <ImageContainer>
            ...
            <MainImage
              ...
              alt={LOCAL_COLORS[activeIndex].name}
            />
        ...
        <RightSection>
          ...
          <OptionLabel>
            ...
            <span style={{ fontWeight: "normal", color: "#666" }}>
              {LOCAL_COLORS[activeIndex].name}
            </span>
          </OptionLabel>
          <ColorGrid>
            {LOCAL_COLORS.map((color, idx) => (
              <ColorThumbnail
                key={idx}
          ...
          <GenderToggle>
            <GenderBtn $active={true}>남성</GenderBtn>
            <GenderBtn $active={false}>여성</GenderBtn>
          </GenderToggle>
          ...
          >
            오프라인 매장 재고 확인! <br />
            <span style={{ fontWeight: "normal", color: "#666" }}>
              사이즈를 선택하시면 재고가 있는 매장을 확인하실 수 있습니다.
            </span>
          </div>
          <AddCartBtn onClick={handleAddToCart} disabled={!selectedSize}>
            {selectedSize ? "장바구니 담기" : "사이즈 선택하기"}
          </AddCartBtn>
        </RightSection>
      </ContentGrid>
